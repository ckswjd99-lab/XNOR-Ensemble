...
if __name__=='__main__':
    ...
    class BaggingModel(nn.Module):
        def __init__(self, num_learners):
            ...
            self.models = nn.ModuleList([ResNet9(bin_active=BIN_ACTIVE) for _ in range(num_learners)])
        ...
        def forward(self, x):
            prediction = torch.zeros(x.size(0), 10).cuda()
            for model in self.models:
                model.eval()
                output = model(x)
    ...
    model = BaggingModel(num_learners=NUM_LEARNERS).cuda()
