...
training_params = list(model.meta.parameters())
weak_params = []
for weak_model in model.colleagues:
    weak_model_params = [param for name, param in weak_model.named_parameters() if 'module.conv1' not in name and 'module.linear' not in name]
    weak_params.extend(weak_model_params)
training_params.extend(weak_params)
