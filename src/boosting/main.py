...
def rate_agent(model, binarizer, train_loader, dweights):
    ...
    stage = torch.log((1 - error) / error + 1e-6) + torch.log(torch.Tensor([NUM_CLASSES - 1])).cuda()
...
def train(epoch, model, binarizer, train_loader, dweights, optimizer, criterion):
    ...
    for batch_idx, (data, target) in pbar:
        ...
        # forwarding
        data, target = data.cuda(), target.cuda()
        batch_dweights = dweights[batch_idx*BATCH_SIZE:min((batch_idx+1)*BATCH_SIZE, len(train_loader.dataset))]
        ...
        loss = torch.dot(loss, batch_dweights) * len(train_loader.dataset)
        loss.backward()
...
if __name__=='__main__':
    ...
    for idx in range(len(model_agents)):
        ...
        model.load_state_dict(torch.load(FORMER_AGENTS[idx]))

        stage, train_dweights, agent_t_loss, agent_t_acc = rate_agent(model, binarizer, train_loader, train_dweights)
        agent_v_loss, agent_v_acc = validate(0, model, test_loader, nn.CrossEntropyLoss(reduce=False).cuda(), binarizer)
        stages.append(stage)
    ...
    new_model.load_state_dict(torch.load('./saves/resnet9_best_agent.pth'))

    new_binarizer = BinOp(new_model)
    new_optimizer = optim.Adam(new_model.parameters(), lr=LR_START)
    ...
    for epoch in range(EPOCH):
        ...
        train_loss, train_acc = train(epoch, new_model, new_binarizer, train_loader, train_dweights, new_optimizer, criterion)
        ...
        ensembled_loss, ensembled_acc = validate_ensembled(
            epoch, model_agents, stages, binarizers, new_model, new_stage, new_binarizer, test_loader, criterion
        )

        print(f"EPOCH {epoch:3d} | T LOSS: {train_loss:.4f}, T ACC: {train_acc*100:.2f}%, V LOSS: {val_loss:.4f}, V ACC: {val_acc*100:.2f}%, STAGE: {new_stage.item():.4f} | ENS V LOSS: {ensembled_loss:.4f}, ENS V ACC: {ensembled_acc*100:.2f}%")
